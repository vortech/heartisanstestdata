...
// online check: http://www.epochconverter.com/
    ...
    t_of_day = mktime(&t);
    ppg_UnixTime = (double)((long)t_of_day) + (ms * 0.01);
...
void peakdetector(double time, int ppg){
    // update current
    int c_ppg = ppg;
    double c_time = time;
    double PEAK_t = 0;
    ...
    // add to OUTPUT DATA
    if (PEAK_v > 0)
//        fprintf(outputFile, "%.2f %d %d\n", PEAK_t, ppg_Value, PEAK);
        fprintf(outputFile, "%.2f %d %.2f %d\n", ppg_UnixTime, ppg_Value, PEAK_t, PEAK_v);
    else
        //fprintf(outputFile, "%.2f %d %s\n", ppg_UnixTime, ppg_Value, "");
        fprintf(outputFile, "%.2f %d %s %s\n", ppg_UnixTime, ppg_Value, "", "");
...
// storage/emulated/0/Dev/heartbeat/c
    ...
    if (f){
        ...
        for (row = 0; row < ROWS && fgets(rowstr, 100, f); ++row){
            ...
            // peak detector
            peakdetector(ppg_UnixTime, ppg_v);
